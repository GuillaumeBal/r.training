...
## ------------------------------------------------------------------
## 3.2.1 Accessing/extracting a part of a data set - case of vectors:
...
## ----------------------------------------
## 3.2.2 When there are several dimensions:
...
class(mat[ , 1])  # ... OK, it's been changed in a vector
...
dataf[dataf$treatment, ]  # Selection of rows using   #$used to identify a row or column name ina bigger datset
...
## --------------------------------------
## 3.2.5 Modification of subsets of data:

## Change as you access:

# Vector:
namedVect[1:3] <- c(27, 28, 29)
namedVect

# Matrix:
mat[c(FALSE, TRUE, TRUE, FALSE),
    c("B", "C")] <- matrix(0, nrow=2, ncol=2)  # "Inclusion" of a matrix of the same size
mat

mat[1:2, 2:3] <- rep(x = NA, times = 4)  # The replacement vector is automatically 
mat                                      # converted in matrix.

mat[1:2, c("A", "B")] <- Inf  # It is even recycled if needed!
mat
...
## -----------------------------------------
## 3.2.7 Indices and names for adding stuff:
...
datasub2 <- subset(x = dataf, 
                   value > 10)  # Note that the name of the column can be directly 
# used as a vector 
...
format(c(date1, date2, date3),  # Formatting dates [call format.Date()].
       format = "%d/%m")
...
order(dataf$value)
dataf[order(dataf$value), ]  # ordering data by treatment, then value.
